...
function extractFirstOpenGraphValue(head, keys) {
  for (const key of keys) {
    const value = extractOpenGraphValue(head, key)
...
export async function fetchOgPreview(rawUrl) {
  ...
  const title = extractFirstOpenGraphValue(head, ['og:title', 'twitter:title'])
  const rawImage = extractFirstOpenGraphValue(head, [
    'og:image',
    'og:image:url',
    'twitter:image',
    'twitter:image:src',
